...
if [ $outgroup = "NoOutgroup" ]; then
	dowork() { 
		...
		b=1`date +%N`
		let "c=$a+$b"
		treemix -i $2 -bootstrap -k $3 -se -m $4 -seed $c -global -o "bootstrap/"$5"_treemix_bootrep_"$1
	}
	export -f dowork
	...
	dowork() { 
		...
		b=1`date +%N`
		let "c=$a+$b"
		treemix -i $2 -bootstrap -k $3 -se -m $4 -root $6 -seed $c -global -o "bootstrap/"$5"_treemix_bootrep_"$1
	}
	export -f dowork
...
if [ $outgroup = "NoOutgroup" ]; then
	treemix -i $infile -m $numk -k $blockk -se -global -tf $outname"_outtree.newick" -o $outname > $outname"_logfile_treemix_boot.log"
else
	treemix -i $infile -m $numk -k $blockk -root $outgroup -se -global -tf $outname"_outtree.newick" -o $outname > $outname"_logfile_treemix_boot.log"
fi
echo "**** RUNNING TREEMIX: DONE ****"
