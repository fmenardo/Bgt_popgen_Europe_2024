...
for (i in 1:length(f)) {
  ...
  MH <- ggplot(gwscan, aes(x = marker, y = tajimasD, color = odd.chr)) +
  ...
  MH_Avr_Svr <- ggplot(gwscan, aes(x = marker, y = tajimasD, color = odd.chr)) +
    ...
    scale_x_continuous(breaks = x.chr, labels = 1:11) +
    scale_y_continuous(limits = c(min(y) - 1, max(y) + 1), expand = c(0,0)) +
    scale_color_manual(values = c("black", "azure4"), guide = "none") +
    labs(x = "", y = "Tajima's D") +
    #geom_hline(yintercept = BC) +
    #geom_hline(yintercept = fdrlog10, linetype = "dotted") +
  ...
  #print(MH_Avr_Svr)
  #dev.off()
  saveRDS(MH_Avr_Svr, paste0("../2_output/plots/MH_TajimasD_",f_short[i],".RDS"))
  png(paste0("../2_output/plots/MH_TajimasD_",f_short[i],".png"), width = 12, height = 4, units = "in", res = 150)
  print(MH_Avr_Svr)
  dev.off()
