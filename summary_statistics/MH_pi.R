...
for (i in 1:length(f)) {
  ...
  n <- length(gwscan$pi_in_window)
  
  # Sort the negative log10(p-values) from largest to smallest.
  y <- rev(sort(gwscan$pi_in_window))
  ...
  MH <- ggplot(gwscan, aes(x = marker, y = pi_in_window, color = odd.chr)) +
  ...
  MH_Avr_Svr <- ggplot(gwscan, aes(x = marker, y = pi_in_window, color = odd.chr)) +
    ...
    geom_text(aes(x=gene_of_int_fungicide[1], label="Btub", y=max(y) - 10), colour="dodgerblue3", angle=60, size = 2.5) +
    geom_text(aes(x=gene_of_int_fungicide[2], label="cyp51", y=max(y) - 10), colour="dodgerblue3", angle=60, size = 2.5) +
    geom_text(aes(x=gene_of_int_fungicide[3], label="sdhB", y=max(y) - 10), colour="dodgerblue3", angle=60, size = 2.5) +
    geom_text(aes(x=gene_of_int_fungicide[4], label="sdhC", y=max(y) - 10), colour="dodgerblue3", angle=60, size = 2.5) +
    geom_text(aes(x=gene_of_int_fungicide[5], label="sdhD", y=max(y) - 10), colour="dodgerblue3", angle=60, size = 2.5) +
    geom_text(aes(x=gene_of_int_Avr_Svr_marker[1], label="AvrPm3d3", y=max(y) - 10), colour="red3", angle=60, size = 2.5) +
    geom_text(aes(x=gene_of_int_Avr_Svr_marker[3], label="AvrPm2", y=max(y) - 10), colour="red3", angle=60, size = 2.5) +
    geom_text(aes(x=gene_of_int_Avr_Svr_marker[4], label="AvrPm3a2/f2", y=max(y) - 10), colour="red3", angle=60, size = 2.5) +
    geom_text(aes(x=gene_of_int_Avr_Svr_marker[5], label="SvrPm3a1/f1", y=max(y) - 10), colour="red3", angle=60, size = 2.5) +
    geom_text(aes(x=gene_of_int_Avr_Svr_marker[6], label="AvrPm3b2/c2", y=max(y) - 10), colour="red3", angle=60, size = 2.5) +
    geom_text(aes(x=gene_of_int_Avr_Svr_marker[7], label="AvrPm1a.1", y=max(y) - 10), colour="red3", angle=60, size = 2.5) +
    geom_text(aes(x=gene_of_int_Avr_Svr_marker[8], label="AvrPm1a.2", y=max(y) - 10), colour="red3", angle=60, size = 2.5) +
    geom_text(aes(x=gene_of_int_Avr_Svr_marker[9], label="AvrPm17", y=max(y) - 10), colour="red3", angle=60, size = 2.5) +
    geom_text(aes(x=gene_of_int_Avr_Svr_marker[11], label="AvrPm8", y=max(y) - 10), colour="red3", angle=60, size = 2.5) +
    geom_text(aes(x=gene_of_int_Avr_Svr_marker[12], label="AvrPm3e", y=max(y) - 10), colour="red3", angle=60, size = 2.5) +
    scale_x_continuous(breaks = x.chr, labels = 1:11) +
    scale_y_continuous(limits = c(-1, max(y) + 10), expand = c(0,0)) +
    scale_color_manual(values = c("black", "azure4"), guide = "none") +
    labs(x = "", y = "pi") +
    #geom_hline(yintercept = BC) +
    #geom_hline(yintercept = fdrlog10, linetype = "dotted") +
  ...
  #print(MH_Avr_Svr)
  #dev.off()
  saveRDS(MH_Avr_Svr, paste0("../2_output/plots/MH_pi_in_window_",f_short[i],".RDS"))
  png(paste0("../2_output/plots/MH_pi_in_window_",f_short[i],".png"), width = 12, height = 4, units = "in", res = 150)
  print(MH_Avr_Svr)
  dev.off()
