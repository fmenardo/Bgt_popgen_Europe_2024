...
#snpEff_path = "/home/jjigis/data/snpEff/snpEff.jar"
...
def mapping(ref,i1,scratch_fol):
    ...
    cmd_map4 = f"{bwa_path} mem -t 2 {str(ref)} {scratch_fol}{str(i1[:-11])}unpaired2.fq.gz > {scratch_fol}aln_{str(i1[:-11])}unpaired2.sam"
...
def mark_dup(i1, scratch_fol):
    ...
    if "renamed" in str(i1):
        ...
        rgsm = str(i1)[:-12]
    cmd_md1 = f"{gatk_path} --java-options '-Xmx12g' AddOrReplaceReadGroups -I {scratch_fol}aln_{str(i1[:-11])}compiled.bam -O {scratch_fol}aln_{str(i1[:-11])}compiled_RG.bam -RGLB lib_{str(i1[:-12])} -RGPL illumina -RGPU PU_{str(i1[:-12])} -RGSM {rgsm}"
    subprocess.run(cmd_md1, shell = True)
    index(f"{scratch_fol}aln_{str(i1[:-11])}compiled_RG.bam")
    cmd_md2 = f"{gatk_path} --java-options '-Xmx12g' MarkDuplicatesSpark -I {scratch_fol}aln_{str(i1[:-11])}compiled_RG.bam -O {scratch_fol}aln_{str(i1[:-11])}compiled_marked_dup.bam -M {scratch_fol}aln_{str(i1[:-11])}marked_dup_metrics.txt --conf 'spark.local.dir={scratch_fol}'"
...
def avg_coverage(scratch_fol, i1):
    ...
    ### should remove duplicates in previous step to get accurate estimate here
    cmd_ac1 = f"{samtools_path} coverage -A {scratch_fol}aln_{str(i1[:-11])}compiled_marked_dup.bam -o {scratch_fol}{str(i1[:-11])}avg_coverage"
...
def hap_caller(ref,scratch_fol,i1):
    ...
    cmd_hc1 = f"{gatk_path} --java-options '-Xmx12g' HaplotypeCaller -R {ref} -I {scratch_fol}aln_{str(i1[:-11])}compiled_marked_dup.bam -O {vcf_fol}{str(i1[:-12])}.bp.vcf.gz -ploidy 1 -ERC BP_RESOLUTION"
    subprocess.run(cmd_hc1, shell = True)
    #subprocess.run(f"{tabix_path} -p vcf {vcf_fol}{str(i1[:-12])}.bp.vcf.gz", shell = True)
...
'''    
def vcf_recoder(i1,scratch_fol,vcf_fol,min_depth):
    if __name__ == '__main__':
        missing_stats = recode_bp_vcf_gatk.recode_vcf(str(i1[:-12]).bp.vcf.gz,scratch_fol,str(i1[:-12]),vcf_fol,min_depth)
...
##parser.add_argument('-mc','--minimum_coverage', type=int, default='', metavar='',  help='minimum coverage threshold to recode as missing data in vcf', nargs=1)
...
mean_coverage = avg_coverage(scratch_folder,i1)
hap_caller(ref,scratch_folder,i1)
section_vcf(i1,scratch_folder)
###vcf_recoder(i1,scratch_folder,vcf_fol,ag.minimum_coverage[0])
extract_mito(i1, scratch_folder)
#filter_var(ref,scratch_folder,i1)
#missing_data_stats = vcf_recoder(i1,scratch_folder)
#annotate_vcf(i1,scratch_folder)
...
'''
print(f"isolate = {i1[:-12]}")
totreads = re.findall(r'\d+',str(total_reads))[0]
print(f"total number of reads = {totreads}")
print(f"number of reads merged due to overlap = {merged}")
print(f"proportion of reads mapped = {str(prop_mapped)}")
print(f"average genome wide coverage = {str(mean_coverage)}")
